...
const qrcode = require('../miniprogram/vendor/qrcode');
const {
  binarizeImageData,
  fitText,
  renderDocument,
  validateDocument
} = require('../miniprogram/core/renderer');
...
test('image thresholding uses the WeChat offscreen canvas API when available', (t) => {
  ...
  const raster = {
    ...
    getContext() {
      return {
        fillRect() {},
        drawImage() {},
        getImageData: () => ({ data: Uint8ClampedArray.from([100, 100, 100, 255]), width: 1, height: 1 }),
        putImageData: (value) => { raster.output = value; }
      };
    }
  };
  global.wx = {
    createOffscreenCanvas(options) {
      created += 1;
      assert.deepEqual(options, { type: '2d', width: 1, height: 1 });
      return raster;
    }
  };
  t.after(() => {
  ...
  const context = createCanvasContext();
  renderDocument(context, {
    elements: [{ id: 'wechat-photo', type: 'image', x: 0, y: 0, width: 1, height: 1, threshold: 180 }]
  }, { width: 1, height: 1 }, 25.4, { 'wechat-photo': {} });
  assert.equal(created, 1);
  assert.equal(context.drawImageCalls[0][0], raster);
  assert.deepEqual(Array.from(raster.output.data), [0, 0, 0, 255]);
});
