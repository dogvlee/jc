...
const { importNiimTemplate, mapAlignH, mapAlignV, parseFontStyle, isMissingImagePath } = require("../miniprogram/core/niim-template-import");
const product = require("../miniprogram/data/niim-templates/product.json");
const medicine = require("../miniprogram/data/niim-templates/medicine-price.json");
const stateGrid = require("../miniprogram/data/niim-templates/state-grid.json");
const c1 = require("../miniprogram/data/niim-templates/c1-default.json");
...
test('imports reject remote and device-local image references from the offline document', () => {
  const { document } = importNiimTemplate({
    width: 30,
    height: 15,
    elements: [
      { type: 'image', imageUrl: 'https://example.com/logo.png', x: 0, y: 0, width: 5, height: 5 },
      { type: 'image', localUrl: '/var/mobile/private.png', x: 5, y: 0, width: 5, height: 5 },
      { type: 'image', localUrl: 'assets/local.png', x: 10, y: 0, width: 5, height: 5 }
    ]
  });
  assert.deepEqual(
    document.elements.filter((element) => element.type === 'image').map((element) => element.path),
    ['assets/local.png']
  );
});

test('imported NIIM dimensions stay inside the supported mini-program range', () => {
  const document = importNiimTemplate({ width: 999, height: 1, elements: [] }).document;
  assert.deepEqual([document.widthMm, document.heightMm], [150, 5]);
});
